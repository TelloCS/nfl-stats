  ...
  <div className="bg-gray-50 border-b border-neutral-200">
    <div className="container mx-auto flex flex-col px-8 py-6 gap-6">
      <div className="flex flex-wrap items-center gap-4">
        <FilterSelect value={filters.season_year} onChange={(v) => onFilterChange('season_year', v)} options={FilterConfig.season_year} minWidth="90px" />
        <FilterSelect value={filters.season_type} onChange={(v) => onFilterChange('season_type', v)} options={FilterConfig.season_type} minWidth="140px" />
        <FilterSelect value={filters.location} onChange={(v) => onFilterChange('location', v)} options={FilterConfig.location} minWidth="110px" />

        <div className="h-8 w-px bg-gray-300 mx-2 hidden md:block"></div>

        <FilterSelect value={filters.opponent} onChange={(v) => onFilterChange('opponent', v)} options={TeamAbbreviations} minWidth="120px" />
        <FilterSelect value={filters.position} onChange={(v) => onFilterChange('position', v)} options={Positions} minWidth="100px" />
      </div>
    ...
    <table className="w-full border-collapse border-spacing-0">
      <thead className="bg-gray-100 text-neutral-600 h-[40px]">
        <tr className="border-b border-neutral-200 uppercase text-xs [&>th]:font-semibold">
          <SortableTh label="Week" sortKey="week" activeSort={sortConfig} onClick={() => onHeaderClick('week')} />
