        ...
        {isPending ? (
          ...
          <div className="container mx-auto px-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              <PlayerPerformanceSection data={data} />
              <MatchupAnalysisSection games={data?.stats} rankingData={rankingData} />
            </div>
          </div>
        )}
      </div>
...
const PlayerPerformanceSection = memo(({ data }) => {
...
const MatchupAnalysisSection = memo(({ games, rankingData }) => {
  ...
  }, [activeCategory, rankingData, teamOne, teamTwo]);
...
const StatToggle = ({ options, activeKey, onSelect }) => (
  <div className="flex gap-1.5 mb-4 pb-2 overflow-x-auto">
    {options.map((opt) => (
      <button
        key={opt.key}
        onClick={() => onSelect(opt.key)}
        className={`whitespace-nowrap px-4 py-2 rounded-md text-xs font-bold transition-colors ${activeKey === opt.key
